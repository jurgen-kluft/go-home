...
func setLightState(w http.ResponseWriter, r *http.Request, p httprouter.Params) {
	...
	response := Response{}
	if hstate, ok := getHueStateByName(p.ByName("lightId")); ok {
		...
		setHueStateByName(p.ByName("lightId"), hstate)
	} else {
		log.Println("[DEVICE] handler not found with name:", p.ByName("lightid"))
		for _, param := range p {
			log.Println("[HTTP-PARAMS] key:", param.Key, " value:", param.Value)
		}
	}
